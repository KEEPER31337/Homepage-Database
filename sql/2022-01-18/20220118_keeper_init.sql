...
-- -----------------------------------------------------
-- Table `keeper`.`static_write`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `keeper`.`static_write` ;

CREATE TABLE IF NOT EXISTS `keeper`.`static_write` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  INDEX `fk_static_write_thumbnail1_idx` (`thumbnail_id` ASC) VISIBLE,
  CONSTRAINT `fk_static_write_static_write_type1`
    FOREIGN KEY (`type`)
    REFERENCES `keeper`.`static_write_type` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_static_write_thumbnail1`
    FOREIGN KEY (`thumbnail_id`)
    REFERENCES `keeper`.`thumbnail` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
